...
*/
#ifndef MXNET_OPERATOR_QUADRANGLE_ROI_POOLING_INL_H_
#define MXNET_OPERATOR_QUADRANGLE_ROI_POOLING_INL_H_
...
namespace roipool {
enum QROIPoolingOpInputs {kData, kBox};
enum QROIPoolingOpOutputs {kOut, kMaxIdx};
}  // roipool

struct QROIPoolingParam : public dmlc::Parameter<QROIPoolingParam> {
  TShape pooled_size;
  float spatial_scale;
  DMLC_DECLARE_PARAMETER(QROIPoolingParam) {
...
class QROIPoolingOp : public Operator {
 public:
  explicit QROIPoolingOp(QROIPoolingParam p) {
    this->param_ = p;
  }
  ...
  virtual void Forward(const OpContext &ctx,
    ...
    out = -FLT_MAX;
    max_idx = -1.0f;
    QROIPoolForward(out, data, bbox, max_idx, param_.spatial_scale);
  }
  ...
  virtual void Backward(const OpContext &ctx,
    ...
    if (kAddTo == req[roipool::kData] || kWriteTo == req[roipool::kData]) {
      ...
      QROIPoolBackwardAcc(grad_in, grad_out, bbox, max_idx, param_.spatial_scale);
    }
 ...
 private:
  QROIPoolingParam param_;
};  // class ROIPoolingOp

// Decalre Factory function, used for dispatch specialization
template<typename xpu>
Operator* CreateOp(QROIPoolingParam param, int dtype);

#if DMLC_USE_CXX11
class QROIPoolingProp : public OperatorProperty {
 public:
  ...
  bool InferShape(std::vector<TShape> *in_shape,
    ...
    CHECK_EQ(bshape.ndim(), 2U) << "bbox should be a 2D tensor of shape [batch, 9]";
    CHECK_EQ(bshape[1], 9U) << "bbox should be a 2D tensor of shape [batch, 9]";
  ...
  OperatorProperty* Copy() const override {
    QROIPoolingProp* roi_pooling_sym = new QROIPoolingProp();
...
#endif  // MXNET_OPERATOR_QUADRANGLE_ROI_POOLING_INL_H_
