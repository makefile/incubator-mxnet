...
inline void QROIPoolForward(const Tensor<cpu, 4, Dtype> &out,
...
template<typename Dtype>
inline void QROIPoolBackwardAcc(const Tensor<cpu, 4, Dtype> &in_grad,
...
template<>
Operator *CreateOp<cpu>(QROIPoolingParam param, int dtype) {
  Operator* op = nullptr;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
    op = new QROIPoolingOp<cpu, DType>(param);
  });
  return op;
}

Operator *QROIPoolingProp::CreateOperatorEx(Context ctx, std::vector<TShape> *in_shape,
                                           std::vector<int> *in_type) const {
  DO_BIND_DISPATCH(CreateOp, param_, in_type->at(0));
}

DMLC_REGISTER_PARAMETER(QROIPoolingParam);

MXNET_REGISTER_OP_PROPERTY(QROIPooling, QROIPoolingProp)
...
.add_arguments(QROIPoolingParam::__FIELDS__());
}  // namespace op
}  // namespace mxnet
