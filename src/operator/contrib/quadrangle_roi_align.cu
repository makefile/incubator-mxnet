...
template <typename T>
__global__ void QRoIAlignForwardKernel(
    const int nthreads,
    const T* bottom_data,
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    // fyk: horizon bbox in feature map
    T P[8];
    ...
    T roi_start_w = fmin(fmin(P[0],P[2]),fmin(P[4],P[6]));
    T roi_start_h = fmin(fmin(P[1],P[3]),fmin(P[5],P[7]));
    T roi_end_w = fmax(fmax(P[0],P[2]),fmax(P[4],P[6]));
    T roi_end_h = fmax(fmax(P[1],P[3]),fmax(P[5],P[7]));
    ...
    for (int iy = 0; iy < roi_bin_grid_h; iy++) {  // e.g., iy = 0, 1
      ...
      for (int ix = 0; ix < roi_bin_grid_w; ix++) {
        ...
        for (int i = 0, j = pn - 1; i < pn; j = i++) {
            T _ix = P[i * 2], _iy = P[i * 2 + 1];
            T _jx = P[j * 2], _jy = P[j * 2 + 1];
            if ( (_iy > y) != (_jy > y) &&
                   x < (_jx - _ix) * (y - _iy) / (_jy - _iy) + _ix ) {
                isInside = !isInside;
            }
        }
...
template <typename T>
__global__ void QRoIAlignBackwardKernel(
    const int nthreads,
    const T* top_diff,
    const int num_rois,
    const T spatial_scale,
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    int count = 0; // this loop only count the valid area points
    for (int iy = 0; iy < roi_bin_grid_h; iy++) {  // e.g., iy = 0, 1
      const T y = roi_start_h + ph * bin_size_h +
          static_cast<T>(iy + .5f) * bin_size_h /
              static_cast<T>(roi_bin_grid_h);  // e.g., 0.5, 1.5
...
.set_attr<FCompute>("FCompute<gpu>", QROIAlignForwardCompute<gpu>);

NNVM_REGISTER_OP(_backward_QROIAlign)
.set_attr<FCompute>("FCompute<gpu>", QROIAlignBackwardCompute<gpu>);
