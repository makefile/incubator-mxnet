 ...
 * Adapted from Caffe2
*/
#include "./quadrangle_roi_align-inl.h"
...
void QROIAlignForwardCompute(const nnvm::NodeAttrs& attrs,
                            const OpContext& ctx,
  ...
  const QROIAlignParam& param = nnvm::get<QROIAlignParam>(attrs.parsed);
  ...
  MSHADOW_REAL_TYPE_SWITCH(in_data[0].type_flag_, DType, {
    ...
    QROIAlignForward<DType>(count, bottom_data, param.spatial_scale, channels,
...
template<typename xpu>
void QROIAlignBackwardCompute(const nnvm::NodeAttrs& attrs,
                             const OpContext& ctx,
  ...
  MSHADOW_REAL_TYPE_SWITCH(out_grad[0].type_flag_, DType, {
    ...
    if (kAddTo == req[roialign::kData] || kWriteTo == req[roialign::kData]) {
      ...
      QROIAlignBackward<DType>(count, top_diff, num_rois, param.spatial_scale,
...
DMLC_REGISTER_PARAMETER(QROIAlignParam);

NNVM_REGISTER_OP(_contrib_QROIAlign)
.describe(R"code(
...
.set_attr_parser(ParamParser<QROIAlignParam>)
.set_attr<nnvm::FInferShape>("FInferShape", [](const nnvm::NodeAttrs& attrs,
  ...
  CHECK_EQ(bshape.ndim(), 2) << "bbox should be a 2D tensor of shape [batch, 9]";
  CHECK_EQ(bshape[1], 9) << "bbox should be a 2D tensor of shape [batch, 9]";
...
.set_attr<FCompute>("FCompute<cpu>", QROIAlignForwardCompute<cpu>)
.set_attr<nnvm::FGradient>("FGradient",
  [](const nnvm::NodePtr& n, const std::vector<nnvm::NodeEntry>& ograds) {
    std::vector<nnvm::NodeEntry> heads;
    heads.push_back(ograds[roialign::kOut]);
    heads.push_back(n->inputs[roialign::kBox]);
    return MakeGradNode("_backward_QROIAlign", n, heads, n->attrs.dict);
  })
...
.add_arguments(QROIAlignParam::__FIELDS__());


NNVM_REGISTER_OP(_backward_QROIAlign)
.set_num_outputs(2)
.set_attr<nnvm::TIsBackward>("TIsBackward", true)
.set_attr_parser(ParamParser<QROIAlignParam>)
.set_attr<FCompute>("FCompute<cpu>", QROIAlignBackwardCompute<cpu>);
