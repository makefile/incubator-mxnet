...
class ProposalRotateOp : public Operator{
 public:
  explicit ProposalRotateOp(ProposalRotateParam param) {
    this->param_ = param;
  }
...
template<>
Operator *CreateOp<cpu>(ProposalRotateParam param) {
  return new ProposalRotateOp<cpu>(param);
}

Operator* ProposalRotateProp::CreateOperator(Context ctx) const {
  DO_BIND_DISPATCH(CreateOp, param_);
}

DMLC_REGISTER_PARAMETER(ProposalRotateParam);

MXNET_REGISTER_OP_PROPERTY(_contrib_ProposalRotate, ProposalRotateProp)
...
.add_arguments(ProposalRotateParam::__FIELDS__());
